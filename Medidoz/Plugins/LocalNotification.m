...
- (void)addNotification:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options {
    ...
    [repeatDict setObject:[NSNumber numberWithInt:NSHourCalendarUnit] forKey:@"alternatedays"];
	...
	if([repeat isEqualToString:@"alternatedays"])
    {
        NSInteger interval = 48 * 60; //date for alternative days
        date = [NSDate dateWithTimeIntervalSinceNow:interval];
       // NSLog(@"Inside Local Notificatio Alternative days %@", date);
    }
    else
    {
...
- (void)cancelNotification:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options {
	...
	for (UILocalNotification *notification in notifications) {
        ...
        NSString *notId = [NSString stringWithFormat:@"%@",[notification.userInfo objectForKey:@"notificationId"]];
